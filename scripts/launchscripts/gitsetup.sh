#!/bin/bash
#################################################
# Script to install git on machine and clone
# our repo into the home dir 
#################################################

# which user to be used
auser=ubuntu
ahome=/home/$auser

apt-get update
apt-get upgrade -y
apt-get install git -y

#setup ssh config
key="-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"

sshconfig="host github.com
  HostName github.com
  IdentityFile ~/.ssh/id_rsa_github
  User git"

hosts="|1|ZQVBF0swqh3i5zBSg4Kwgbt4/XI=|hQIGxNO4T6olF7jfTQ0a1LHolbI= ssh-rsa 
|1|r+W4IcysXkngiJN9OSMXiHpL+O0=|mU6uA/vUxIGrxJn1iGFsvFSIPdg= ssh-rsa "



echo "$key"  | sudo -u $auser tee  $ahome/.ssh/id_rsa_github
sudo -u $auser chmod 600 $ahome/.ssh/id_rsa_github
echo "$sshconfig" | sudo -u $auser tee $ahome/.ssh/config
echo "$hosts" | sudo -u $auser tee $ahome/.ssh/known_hosts

#clone repo
cd $ahome
sudo -u $auser git clone git@github.com:xnbya/TeamDisrupt.git
cd TeamDisrupt

#for testing branch
sudo -u ubuntu git checkout scripting

